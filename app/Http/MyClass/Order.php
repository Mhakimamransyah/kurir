...
class Order{
  ...
  public function order_baru($param){
        ...
        // NOTIFIKASI FCM KURIR DISINI
           $notif = Notification::send_notif($id_kurir,"kurir",["data" => [],"title" => "Pesanan Baru", "body" => "Ada Pesanan Baru"]);
     ...
     public function periksa_sesi_order($param){
      $order = Order_Kurir::where("id_order",$param["id_order"])->latest()->first();
        // periksa apakah pelanggan telah membatalknannya atau belum
        // periksa expiration date jika masih valid (tidak sama dengan 0) order masih aktif

      if($order->expired_count > 0){
        $status_order = Order_m::where("id_order",$param["id_order"])->first()->status;

        if($status_order != "pelanggan_batal"){
          ...
          if($order->aksi == "Baru"){
            ...
            return [
                ...
                if($id_kurir != -1){
                  ...
                  // tidak ada kurir lain
                   ...
                   return [
                     ...
                     "kondisi_order" => false, // sesi order batal
                ...
                // kurir setuju tapi di charge
                 ...
                 return [
                     ...
                     "kondisi_order" => true, // sesi order batal
            ...
            //pelanggan membatalkan
                return [
                   "status_order" => 6, // baru
                   "kondisi_order" => false, // sesi order lanjut
                   "counting" => $order->expired_count,
                   "message_order" => "Batal"
                 ];
               }
          ...
          // sesi sudah habis kurir tidak memberikan jawaban
               Order_Kurir::where("id_order",$param["id_order"])->update([
                "aksi" => "Tolak"
              ]);
               $routing = new Routing_kurir();
               $id_kurir = $routing->cari_kurir($param,"penolakan");
      ...
      // in transaction
                 ...
                 Notification::send_notif($param["id_kurir"],"kurir",["data" => ["status" => "kurir_charge"],"title" => "Tarif charge di setujui", "body" => "Charge di setujui pelanggan"]);
      //FCM di SINI
               }
               ...
               public function cancel_order($param){
                 ...
                 if($order->locked != "ya"){
                  ...
                  if($param["id_kurir"] != -1){
                    Notification::send_notif($param["id_kurir"],"kurir",["data" => ["status" => "pelanggan_batal"],"title" => "Pesanan dibatalkan", "body" => "Ada pesanan yang dibatalkan pelanggan"]);
                  }else{
                    // cari kurir berdasarkan id order dulu
                    $id_kurir = Order_Kurir::where("id_order",$param["id_order"])->orderBy("id_order_kurir","DESC")->first()->id_kurir;
                    Notification::send_notif($id_kurir,"kurir",["data" => ["status" => "pelanggan_batal"],"title" => "Pesanan dibatalkan", "body" => "Ada pesanan yang dibatalkan pelanggan"]);
                  }
                  
                }else{
                  throw new \App\Exceptions\MyException([
                   "message" => "Pesanan telah terkunci, silahkan hubungi kurir untuk membatalkan pesanan"
                 ]);
                } 
       // FCM di SINI
              }
      ...
      // PERIKSA APAKAH INI DESTINASI TERAKHIR YANG DI SELESAIKAN
       ...
       //var_dump($jumlah_order." ".$detail_selesai->count());
        ...
        // ORDER SELESAI
                $id_pelanggan = Order_m::find($param["id_order"])->pelanggan->id_pelanggan;
                ...
                $notif = Notification::send_notif($id_pelanggan,"pelanggan",["data" => $param["id_order"],"title" => "Seluruh pesanan sudah selesai", "body" => "Yuk beri rating untuk menyelesaikan status pesanan anda :)"]);
        ...
        // MASIH ADA ORDER LAIN
                ...
                $notif = Notification::send_notif($id_pelanggan,"pelanggan",["data" => $param["id_order"],"title" => "Ada Pesanan Selesai", "body" => "Sebuah destinasi pesanan anda telah sampai "]);
