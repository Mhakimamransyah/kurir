      ...
      if($result->first()->jenis_registrasi == "Manual"){
       ...
       if(Hash::check($request->input('password'),$password_hash)){
        ...
        if($result->first()->is_activate == "yes"){
         ...
         // set kurir logged in
         Kurir::where("id_kurir",$data->id_kurir)->update(["sesi_login_aktif"=>"ya"]);
 ...
 if ($validator->fails()) {
  ...
  $data = [ 
       ...
       // periksa apakah email ada dan teregistrasi secara manual
           ...
           $id_user = $user->first()->id_user;
           $pelanggan = Pelanggan::where("id_user",$id_user)->where("nomor_hp_pelanggan",$request->no_hp)->first();
           $kurir = Kurir::where("id_user",$id_user)->where("no_hp_kurir",$request->no_hp)->first();

           if(($user->exists() && $pelanggan != null && $kurir == null) || ($user->exists() && $pelanggan == null && $kurir != null)){
          ...
          // terdaftar menggunakan google sign in 
           ...
           return ResponseBuilder::result(false,"Data tidak di temukan",[],400);
         }
       }
